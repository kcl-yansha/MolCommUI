...
# ------------------------------------------------------
# Get the default dictionary for the controller settings
def _default_controller_dictionary(key=None):
...
def _init_default_controller_config(file_path):

    # Initialise the init parser
    config = configparser.RawConfigParser()

    # Get the first information in the init file
    config['Default PD'] = _default_controller_dictionary()
...
def _open_config_path(file_name='controller_types.ini'):
    ...
    if not os.path.exists(file_path):
        _init_default_controller_config(file_path=file_path)
...
def getDefaultCalibration():
    return _default_controller_dictionary()

# ------------------------------
# Delete the selected controller
def deletePIDController(config_name, file_name='controller_types.ini'):
...
# -----------------------------------------
# Get the list of controllers from the file
def getPIDList(file_name='controller_types.ini'):
    ...
    controller_list = getSections(config_content)

    return controller_list

# ----------------------------------------------
# Get all the infos of controllers from the file
def getPIDInfos(file_name='controller_types.ini'):
    ...
    # Retrieve the sections
    controller_list = config2dict(config_content)

    # Check the content
    for controller_name in controller_list.keys():
        controller_list[controller_name] = checkValues(controller_list[controller_name], _default_controller_dictionary())

    return controller_list

# --------------------------------------------------------------
# Save the controller calibration config in the user config file
def savePIDConfig(config_name, config_dict, file_name='controller_types.ini'):
...
def loadPIDCalibrationConfig(config_name, file_name='controller_types.ini'):
    ...
    config_dict = checkValues(config_dict, _default_controller_dictionary())
